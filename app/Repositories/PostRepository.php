<?php
  ...
  class PostRepository
  {
    ...
    static function filter( $filterData, $loggedUserId = 0 )
    {
	  ...
	  // todo: only filter posts in public pod or private pod if user is approval in that pod
	  if($loggedUserId){
		$query->join('pods' , 'pods.id', '=', 'posts.pod_id');
		$query->leftJoin('pods_users', function($join) use($loggedUserId){
			$join->on('pods_users.pod_id', '=' ,'pods.id');
			$join->where('pods_users.user_id', '=' , $loggedUserId);
		});
		$query->whereRaw('( (pods.is_private = 0) OR ( pods_users.id IS NOT NULL AND (pods_users.is_approved = 1 OR pods_users.is_owner = 1) ) )');
	  }
	  else{
		  $query->join('pods' , 'pods.id', '=', 'posts.pod_id');
		  $query->whereRaw('pods.is_private = 0');
	  }
      ...
      foreach( $postIds as $postId ) {
		...
		if ( isset( $filterData['pod_id'] ) && $filterData['pod_id'] ) {
		// todo: make as read this post
			Read::makeAsRead($post, $loggedUserId);
		}
